...
blackboxConfig_t blackboxConfig =
{
    ...
    1, //.rate_denom = 
    0, //.device = 
    BLACKBOX_INTERVED_CARD_DETECTION //.invertedCardDetection = 
};
/*
...
static const blackboxDeltaFieldDefinition_t blackboxMainFields[] = {
    ...
    {"cycleTime",       -1, UNSIGNED,.Ipredict = PREDICT(0),.Iencode = ENCODING(UNSIGNED_VB),.Ppredict = PREDICT(PREVIOUS),.Pencode = ENCODING(SIGNED_VB), CONDITION(ALWAYS)},
        /*
...
*/
    {"vario",     -1, SIGNED,.Ipredict = PREDICT(0),.Iencode = ENCODING(SIGNED_VB),.Ppredict = PREDICT(PREVIOUS),.Pencode = ENCODING(SIGNED_VB), CONDITION(ALWAYS)},
    {"targetVario",     -1, SIGNED,.Ipredict = PREDICT(0),.Iencode = ENCODING(SIGNED_VB),.Ppredict = PREDICT(PREVIOUS),.Pencode = ENCODING(SIGNED_VB), CONDITION(ALWAYS)},
    ...
    {"fwAltOut",   -1, SIGNED,   .Ipredict = PREDICT(0),       .Iencode = ENCODING(SIGNED_VB),   .Ppredict = PREDICT(PREVIOUS),      .Pencode = ENCODING(SIGNED_VB), CONDITION(ALWAYS)},
    {"fwAltToHold",-1, SIGNED,.   Ipredict = PREDICT(0),       .Iencode = ENCODING(SIGNED_VB),.Ppredict = PREDICT(PREVIOUS),         .Pencode = ENCODING(SIGNED_VB), CONDITION(ALWAYS)},
    /*
...
*/
    ...
    {"vbat",       -1, UNSIGNED, .Ipredict = PREDICT(0), .Iencode = ENCODING(UNSIGNED_VB),   .Ppredict = PREDICT(PREVIOUS),      .Pencode = ENCODING(TAG8_8SVB), CONDITION(ALWAYS)},
//    {"amperage",   -1, SIGNED,   .Ipredict = PREDICT(0),       .Iencode = ENCODING(SIGNED_VB),   .Ppredict = PREDICT(PREVIOUS),      .Pencode = ENCODING(TAG8_8SVB), FLIGHT_LOG_FIELD_CONDITION_AMPERAGE},
    ...
    {"magADC",      0, SIGNED,   .Ipredict = PREDICT(0),       .Iencode = ENCODING(SIGNED_VB),   .Ppredict = PREDICT(PREVIOUS),      .Pencode = ENCODING(TAG8_8SVB), CONDITION(ALWAYS)},
    {"magADC",      1, SIGNED,   .Ipredict = PREDICT(0),       .Iencode = ENCODING(SIGNED_VB),   .Ppredict = PREDICT(PREVIOUS),      .Pencode = ENCODING(TAG8_8SVB), CONDITION(ALWAYS)},
    {"magADC",      2, SIGNED,   .Ipredict = PREDICT(0),       .Iencode = ENCODING(SIGNED_VB),   .Ppredict = PREDICT(PREVIOUS),      .Pencode = ENCODING(TAG8_8SVB), CONDITION(ALWAYS)},

    {"BaroAlt",    -1, SIGNED,   .Ipredict = PREDICT(0),       .Iencode = ENCODING(SIGNED_VB),   .Ppredict = PREDICT(PREVIOUS),      .Pencode = ENCODING(TAG8_8SVB), CONDITION(ALWAYS)},
 /*
#ifdef USE_PITOT
...
#endif
*/
    {"rssi",       -1, UNSIGNED, .Ipredict = PREDICT(0),       .Iencode = ENCODING(UNSIGNED_VB), .Ppredict = PREDICT(PREVIOUS),      .Pencode = ENCODING(TAG8_8SVB), CONDITION(ALWAYS)},
...
*/
    ...
    {"motor",       0, UNSIGNED, .Ipredict = PREDICT(0), .Iencode = ENCODING(UNSIGNED_VB), .Ppredict = PREDICT(AVERAGE_2), .Pencode = ENCODING(SIGNED_VB), CONDITION(ALWAYS)},
    { "motor",       1, UNSIGNED,.Ipredict = PREDICT(MOTOR_0),.Iencode = ENCODING(SIGNED_VB),.Ppredict = PREDICT(AVERAGE_2),.Pencode = ENCODING(SIGNED_VB), CONDITION(ALWAYS) },
    { "motor",       2, UNSIGNED,.Ipredict = PREDICT(MOTOR_0),.Iencode = ENCODING(SIGNED_VB),.Ppredict = PREDICT(AVERAGE_2),.Pencode = ENCODING(SIGNED_VB), CONDITION(ALWAYS) },
    { "motor",       3, UNSIGNED,.Ipredict = PREDICT(MOTOR_0),.Iencode = ENCODING(SIGNED_VB),.Ppredict = PREDICT(AVERAGE_2),.Pencode = ENCODING(SIGNED_VB), CONDITION(ALWAYS) },
...
/*
    ...
    {"motor",       2, UNSIGNED, .Ipredict = PREDICT(MOTOR_0), .Iencode = ENCODING(SIGNED_VB),   .Ppredict = PREDICT(AVERAGE_2),     .Pencode = ENCODING(SIGNED_VB), CONDITION(ALWAYS)},
    {"motor",       3, UNSIGNED, .Ipredict = PREDICT(MOTOR_0), .Iencode = ENCODING(SIGNED_VB),   .Ppredict = PREDICT(AVERAGE_2),     .Pencode = ENCODING(SIGNED_VB), CONDITION(ALWAYS)},
    {"motor",       4, UNSIGNED, .Ipredict = PREDICT(MOTOR_0), .Iencode = ENCODING(SIGNED_VB),   .Ppredict = PREDICT(AVERAGE_2),     .Pencode = ENCODING(SIGNED_VB), CONDITION(ALWAYS)},
*/
/*
...
static bool testBlackboxConditionUncached(FlightLogFieldCondition condition)
{
    ...
    case FLIGHT_LOG_FIELD_CONDITION_AT_LEAST_MOTORS_8:
        ...
        return 4 >= condition - FLIGHT_LOG_FIELD_CONDITION_AT_LEAST_MOTORS_1 + 1;
...
static void blackboxBuildConditionCache(void)
{
    ...
    for (FlightLogFieldCondition cond = FLIGHT_LOG_FIELD_CONDITION_FIRST; cond <= FLIGHT_LOG_FIELD_CONDITION_LAST; cond = static_cast<FlightLogFieldCondition>(cond + 1)) {
...
static void writeIntraframe(void)
{
    ...
    blackboxWriteUnsignedVB(blackboxCurrent->cycleTime);
 ...
 */
 ...
 /*
    ...
    */
    ...
    blackboxWriteUnsignedVB(blackboxCurrent->rcCommand[3]);
    ...
    /*
      ...
      */
    blackboxWriteUnsignedVB(blackboxCurrent->vbat);
...
//    blackboxWriteUnsignedVB(blackboxCurrent->motor[0] - MINCOMMAND);
...
static void writeInterframe(void)
{
    ...
    blackboxWriteSignedVB(blackboxCurrent->cycleTime - blackboxLast->cycleTime);
    ...
    */
    ...
    blackboxWriteSignedVB(blackboxCurrent->vario - blackboxLast->vario);
    blackboxWriteSignedVB(blackboxCurrent->targetVario - blackboxLast->targetVario);
        ...
        blackboxWriteSignedVB(blackboxCurrent->fwAltToHold - blackboxLast->fwAltToHold);
...
/*
    ...
    */
    ...
    blackboxWriteArrayUsingAveragePredictor16(offsetof(blackboxMainState_t, gyroADC), 3);
    blackboxWriteArrayUsingAveragePredictor16(offsetof(blackboxMainState_t, accADC), 3);
    blackboxWriteArrayUsingAveragePredictor16(offsetof(blackboxMainState_t, attitude), 3);
/*
    if (testBlackboxCondition(FLIGHT_LOG_FIELD_CONDITION_DEBUG)) {
    ...
    */
    ...
    blackboxWriteArrayUsingAveragePredictor16(offsetof(blackboxMainState_t, motor), 4);
    /*
    if (testBlackboxCondition(FLIGHT_LOG_FIELD_CONDITION_SERVOS)) {
...
void blackboxStart(void)
{
  Serial.println("Blackbox:start");
    ...
    vbatReference = analog.vbat;//getBatteryRawVoltage();
    ...
    //Is rc channel state defined for activating BLACKBOX? - leave it False
...
void blackboxFinish(void)
{
  ...
  Serial.println("BLACKBOX:finish()");
...
static void loadMainState(timeUs_t currentTimeUs)
{
    ...
    if (lastUpdateTime == 0)
    {
      blackboxCurrent->cycleTime = 0;
    }
    else
    {
      blackboxCurrent->cycleTime = currentTimeUs - lastUpdateTime;
    }
    ...
    for (int i = 0; i < 3; i++) {
      ...
      // /8192*2000 - in deg
      // /8192*2000 / 180 *314 = 0.425889 = 27911/65536  - in rad*100
      blackboxCurrent->gyroADC[i] = ((int32_t)imu.gyroADC[i])*27911/65536;


        blackboxCurrent->accADC[i] = imu.accSmooth[i];
        blackboxCurrent->magADC[i] = imu.magADC[i];
/*
#ifdef USE_NAV
...
*/
    ...
    blackboxCurrent->vario = alt.estVario;
    blackboxCurrent->targetVario = targetVario;

    blackboxCurrent->fwAltPID[0] = fwAltPID[0];
    blackboxCurrent->fwAltPID[1] = fwAltPID[1];
    blackboxCurrent->fwAltPID[2] = fwAltPID[2];
    blackboxCurrent->fwAltPIDOutput = fwAltPID[3];

    blackboxCurrent->fwAltToHold = altToHold;
...
*/
    ...
    //1800 - 180deg
    //need: 314 - 180 deg
    blackboxCurrent->attitude[0] = ((int32_t)att.angle[0])*3141/18000;
    blackboxCurrent->attitude[1] = ((int32_t)att.angle[1])* 3141 / 18000;
    blackboxCurrent->attitude[2] = ((int32_t)att.angle[2])* 3141 / 18000;
/*
    for (int i = 0; i < DEBUG32_VALUE_COUNT; i++) {
        blackboxCurrent->debug[i] = debug[i];
    }
    */
    const int motorCount = 4;// getMotorCount();
    for (int i = 0; i < motorCount; i++) {
      blackboxCurrent->motor[i] = motor[i];
    }

    blackboxCurrent->vbat = analog.vbat;// getBatteryRawVoltage();
    //blackboxCurrent->amperage = getAmperage();

    blackboxCurrent->BaroAlt = alt.estAlt;
...
#endif
     */
    blackboxCurrent->rssi = analog.rssi;//getRSSI();
...
//        if (IS_RC_MODE_ACTIVE(BOXBLACKBOX) && blackboxShouldLogIFrame()) {
        if (true && blackboxShouldLogIFrame()) {
